    ...
    <?php if ($editing == 1): ?>
        ...
        <div style="text-align: left;" class="featured-node-copy">
            <table style="width: 100%;" border=0>
                ...
                <tbody>
                <tr>
                    <td class="dropSpaceBin" ondragover="return DragOver(event)" ondragleave="return DragLeave(event)"
                        ondrop="return doDrop(event)" rowspan="3">
                        <img id="bin" src="../assets/bin.png" style="height: 50px;"/>
                    </td>
                    <td class="dropSpace">
                        <span style="font-size: 12px; font-weight: bold;">Participants</span>
                        <ul id="users" class="dropable_lists" ondragover="return DragOver(event)"
                            ondragleave="return DragLeave(event)" ondrop="return doDrop(event)">
                            <?php
                            foreach ($run->getParticipants() as $participant) {
                                ?>
                                <li id="db_users_<?php print($participant->getUserID()); ?>" draggable='true'
                        ...
                        <ul>
                            <?php for ($i = 0; $i < count($userList[0]); $i++) { ?>
                                <?php if (isset($userList[0][$i])): ?>
                                    <li id='users_<?php print($userList[0][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
                                        <?php print($userList[0][$i]->getName()); ?>
                                    </li>
                                <?php endif; ?>
                                <?php if (isset($userList[1][$i])): ?>
                                    <li id='users_<?php print($userList[1][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
                                        <?php print($userList[1][$i]->getName()); ?>
                                    </li>
                                <?php endif; ?>
                                <?php if (isset($userList[2][$i])): ?>
                                    <li id='users_<?php print($userList[2][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
                ...
                <tr>
                    <td class="dropSpace">
                        ...
                        <ul id="items" class="dropable_lists" ondragover="return DragOver(event)"
                            ondragleave="return DragLeave(event)" ondrop="return doDrop(event)">
                            <?php
                        ...
                        <ul>
                            <?php for ($i = 0; $i < count($itemList[0]); $i++) { ?>
                                <?php if (isset($itemList[0][$i])): ?>
                                    <li id='items_<?php print($itemList[0][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
                                        <?php print($itemList[0][$i]->getName()); ?>
                                    </li>
                                <?php endif; ?>
                                <?php if (isset($itemList[1][$i])): ?>
                                    <li id='items_<?php print($itemList[1][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
                                        <?php print($itemList[1][$i]->getName()); ?>
                                    </li>
                                <?php endif; ?>
                                <?php if (isset($itemList[2][$i])): ?>
                                    <li id='items_<?php print($itemList[2][$i]->getId()); ?>' draggable='true'
                                        ondragstart='return drag(event)' class="floatListItem">
